...
// 2. 載入第二層社團選單 (徹底避免 HTML 錯誤拋錯導致載入失敗)
function fetchClubCards() {
  ...
  const targetUrl = `${GAS_WEB_APP_URL}?action=getClubs&category=${encodeURIComponent(category)}`;
  ...
  fetch(targetUrl, { redirect: "follow" })
    ...
    .then(text => {
      ...
      try {
        ...
        throw new Error("後端未回傳合法 JSON 格式：" + text.substring(0, 100));
      }
